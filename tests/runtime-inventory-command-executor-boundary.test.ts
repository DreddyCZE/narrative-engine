...
type RuntimeInventoryCommandExecutionInput = engineContracts.RuntimeInventoryCommandExecutionInput;
...
function createInventoryExecutionInput(
  plan: RuntimeCommandPlan,
  actionAffordances?: readonly ContentActionAffordance[],
  inventoryItemIds?: readonly string[]
): RuntimeInventoryCommandExecutionInput {
  return {
    plan,
    ...
    metadata: {
      requestId: "req.task-087.sample",
      correlationId: "corr.task-087.sample",
      deterministic: true
    }
  };
}
...
describe("runtime inventory command executor boundary", () => {
  it("executes planned inventory read-only", () => {
    expect(RUNTIME_INVENTORY_COMMAND_EXECUTION_STATUSES).toEqual(["executed", "rejected", "blocked"]);
    ...
    const input = createInventoryExecutionInput(plan);
    const beforePlan = canonicalizeJson(plan);
    const beforeState = canonicalizeJson(input.playerState);

    const result = executeRuntimeInventoryCommand(input);
  ...
  it("executes empty inventory read-only", () => {
    const plan = createRuntimeCommandPlan({
      request: { commandId: "inventory" },
      content: createReadModel(undefined, []),
      playerState: createPlayerState(undefined, [])
    });

    const result = executeRuntimeInventoryCommand(createInventoryExecutionInput(plan, undefined, []));
  ...
  it("rejects non-inventory planned commands", () => {
    const plan = createRuntimeCommandPlan({
    ...
    expect(result.diagnostics).toEqual([
      {
        ...
        code: "RUNTIME_INVENTORY_COMMAND_EXECUTOR_COMMAND_UNSUPPORTED",
        severity: "error",
        category: "validation",
        phase: "command-execution",
        path: ["plan", "commandId"],
        message: "only the inventory command is supported by this executor.",
        source: {
  ...
  it("blocks non-planned inventory plans", () => {
    const blockedPlan = createRuntimeCommandPlan({
      request: { commandId: "inventory" },
      content: createReadModel(["look", "go", "talk", "take", "save", "load"]),
      playerState: createPlayerState(["look", "go", "talk", "take", "save", "load"])
    });

    const result = executeRuntimeInventoryCommand(
      createInventoryExecutionInput(blockedPlan, ["look", "go", "talk", "take", "save", "load"])
    );

    expect(result.status).toBe("blocked");
    expect(result.view).toBeUndefined();
    expect(result.diagnostics.map((diagnostic) => diagnostic.code)).toContain("RUNTIME_INVENTORY_COMMAND_EXECUTOR_PLAN_NOT_EXECUTABLE");
  });

  it("blocks unresolved inventory items", () => {
    const plan = createRuntimeCommandPlan({
      request: { commandId: "inventory" },
      content: createReadModel(),
      playerState: createPlayerState()
  ...
  it("validates invalid input", () => {
    ...
    const diagnostics = inspectRuntimeInventoryCommandExecutionInput(invalidInput);

    expect(diagnostics.map((diagnostic) => diagnostic.code)).toContain("RUNTIME_INVENTORY_COMMAND_EXECUTION_METADATA_INVALID");
    expect(diagnostics.map((diagnostic) => diagnostic.code)).toContain("RUNTIME_PLAYER_STATE_INVENTORY_ITEM_INVALID");
    expect(() => { assertRuntimeInventoryCommandExecutionInput(invalidInput); }).toThrow();
    expect(formatRuntimeInventoryCommandExecutionValidationMessage([])).toBe("Runtime inventory command execution input is valid.");
  });
  ...
  it("keeps execution deterministic and free of next-state outputs", () => {
    ...
    const first = executeRuntimeInventoryCommand(input);
    const second = executeRuntimeInventoryCommand(input);

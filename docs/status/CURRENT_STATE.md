...
**Active task:** none
**Status:** TASK-037 through TASK-060 are DONE. M2 gate verdict is `M2_GATE_PASS_WITH_DEFERRED_ITEMS`. M3 gate verdict is `M3_GATE_PASS_WITH_DEFERRED_ITEMS`. M4 gate verdict is `M4_GATE_PASS_WITH_DEFERRED_ITEMS`. M5 gate verdict is `M5_GATE_PASS_WITH_DEFERRED_ITEMS`. M6 gate verdict is `M6_GATE_PASS_WITH_DEFERRED_ITEMS`.

## Current Workflow

1. **Current milestone:** M7 Production Storage Adapter / Replay Planning.
2. **Current state:** TASK-053 through TASK-060 are DONE. M7 planning is accepted. There is no active task.
3. **Single next most important task:** Create the accelerated `TASK-061/TASK-062` contracts batch.
4. **What the current scope must not change:** the next run may create `TASK-061` and `TASK-062`, but they must remain data-only contracts. No production file IO, no concrete file adapter, no database adapter, no external storage adapter, no replay runtime behavior, no UI/editor, no gameplay/P0 content, and no plugin runtime may be introduced.
5. **How completion is recognized:** TASK-060 is accepted on `main`, validation is green, and `TASK-061` and `TASK-062` have not been created yet.
...
- Next task after acceptance:
  ...
  - `TASK-062 - Serialization and schema version contracts` not created
